...
import { Plus, Receipt, Download, Send, Eye, ShieldCheck } from "lucide-react";
...
function InvoiceDetailModal({ invoice, onClose }: { invoice: any; onClose: () => void }) {
  return (
    <Modal open onClose={onClose} title={`Facture ${invoice.number}`} size="lg"
      footer={<><Button variant="ghost" onClick={onClose}>Fermer</Button><Button><Download className="h-4 w-4" /> Télécharger PDF</Button></>}>
