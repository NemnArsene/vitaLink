...
import {
  ...
  HOSPITAL, AUDIT_LOGS, PHARMACY, computeDashboardKPI,
} from "@/mocks/seed";
import { faker } from "@/lib/faker";
import type {
  Patient, Consultation, MedicalAct, Invoice, Refund,
...
export const getAuditLogs = (params?: {
  ...
  let logs = [...AUDIT_LOGS];
  if (params?.actorId) logs = logs.filter(l => l.actorId === params.actorId);
...
export const getAuditStats = () => {
  ...
  const byAction = AUDIT_LOGS.reduce((acc, l) => {
    acc[l.action] = (acc[l.action] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);
  const successCount = AUDIT_LOGS.filter(l => l.status === "SUCCESS").length;
  const failureCount = AUDIT_LOGS.filter(l => l.status === "FAILURE" || l.status === "ERROR").length;
  return { total, byAction, successCount, failureCount };
};

// ============== PHARMACY ==============
export const getPharmacy = (): PharmacyItem[] => [...PHARMACY];
export const getPharmacyItem = (id: string) => PHARMACY.find(p => p.id === id);
export const updatePharmacyStock = (id: string, stock: number) => {
  const idx = PHARMACY.findIndex(p => p.id === id);
  if (idx === -1) throw new Error("Item not found");
  PHARMACY[idx] = { ...PHARMACY[idx], stock, updatedAt: new Date().toISOString() };
  return PHARMACY[idx];
};
