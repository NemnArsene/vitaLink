    ...
    },
    {
      ...
      cell: (c: Contract) => (
        <div className="w-32">
          ...
          <div className="mt-1">
            <Progress 
              value={( (c.coverageAmount - c.remainingCoverage) / c.coverageAmount ) * 100} 
              size="sm" 
              variant={c.remainingCoverage < c.coverageAmount * 0.1 ? "danger" : "brand"}
            />
          </div>
          <p className="mt-0.5 text-[9px] text-slate-500">Restant: {formatCurrency(c.remainingCoverage)}</p>
        </div>
      ),
      align: "right" as const,
    },
