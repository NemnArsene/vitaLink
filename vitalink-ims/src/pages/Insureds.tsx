...
import { z } from "zod";
import { Plus, Search, Filter, Download, Eye, Edit, Trash2, Mail, Phone, Calendar } from "lucide-react";
...
export function Insureds() {
  ...
  const filtered = insureds.filter((i) => {
    ...
    const matchDate = (!dateStart || join >= new Date(dateStart)) && (!dateEnd || join <= new Date(dateEnd + "T23:59:59"));
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <>
            <Button variant="outline" icon={<Download className="h-4 w-4" />} onClick={handleExport}>Exporter</Button>
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-6">
            ...
            <div className="relative">
              ...
              <input type="date" value={dateStart} onChange={e => setDateStart(e.target.value)}
                className="w-full rounded-lg border border-slate-200 bg-white py-2 pl-8 pr-3 text-xs dark:border-slate-800 dark:bg-slate-950"
                placeholder="Début" title="Date début affiliation" />
            </div>
            <div className="relative">
              <Calendar className="absolute left-2.5 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 pointer-events-none" />
              <input type="date" value={dateEnd} onChange={e => setDateEnd(e.target.value)}
                className="w-full rounded-lg border border-slate-200 bg-white py-2 pl-8 pr-3 text-xs dark:border-slate-800 dark:bg-slate-950"
                placeholder="Fin" title="Date fin affiliation" />
            </div>
          </div>
